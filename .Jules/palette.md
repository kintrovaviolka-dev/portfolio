...
## 2025-10-24 - [Visual Interaction Hints for Keyboard Users]
**Learning:** Custom structural interactive components (e.g., flip cards) often rely on visual cues like `:hover` transforms for mouse users. However, sighted keyboard users navigating via Tab may only receive a generic focus ring on `:focus-visible`, leaving them without explicit visual hints (like keyboard shortcuts) on how to interact with the focused element.
**Action:** Provide explicit visual interaction hints (e.g., `<span class="kbd-hint">Press <kbd>Space</kbd> to flip</span>`) within the component. Ensure these hints are visually hidden by default and only become visible when the interactive parent element receives `:focus-visible`. Additionally, wrap these hints in `aria-hidden="true"` if the interaction is already announced to screen readers.
