...
## 2026-10-27 - [Programmatic Focus on Hash Link Navigation]
**Learning:** When navigating via hash links in a single-page application (e.g., `<a href="#about">`), browsers typically scroll to the target section but do not automatically move keyboard focus to it. This can be problematic for keyboard and screen reader users who expect the focus to follow the visual scroll.
**Action:** Intercept the click event on hash links, extract the target ID, apply `tabindex="-1"` to the target section (if it doesn't already have a tabindex), and call `.focus({ preventScroll: true })` programmatically. Ensure the programmatic focus outline on the section wrapper is suppressed in CSS (`section:focus, section:focus-visible { outline: none; }`).
