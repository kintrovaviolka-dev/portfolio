...
## 2026-08-09 - [Interaction Hints for Hover-Based Components]
**Learning:** For custom interactive components that rely on `:hover` for mouse users (e.g., flip cards), provide explicit interaction hints to guide users. If adding custom CSS is prohibited by design constraints, use native HTML attributes like `title="Press Space or click to flip"` as a fallback mechanism for tooltips to ensure discoverability without breaking layout constraints.
**Action:** When creating structural elements that function as interactive toggles via hover, add a `title` attribute to provide explicit instructions for keyboard and mouse users on how to interact with it.
