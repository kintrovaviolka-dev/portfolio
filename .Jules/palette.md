...
## 2026-08-31 - [Hash Link Navigation Focus]
**Learning:** When navigating via hash links in a single-page application (e.g., `<a href="#about">`), browsers typically scroll to the target section but do not automatically move keyboard focus to it.
**Action:** Intercept the click event, extract the target ID, apply `tabindex="-1"` to the target section, and call `.focus({ preventScroll: true })` programmatically. Ensure the programmatic focus outline on the section wrapper is suppressed in CSS (`section:focus, section:focus-visible { outline: none; }`).
