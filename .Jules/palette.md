...
## 2026-06-17 - [Mobile Off-Canvas Menu Click Outside]
**Learning:** For mobile off-canvas navigation menus, users expect the menu to close when clicking outside of it. However, if a document-level click listener is implemented without explicitly checking that the click target is not the toggle button itself (e.g., `!hamburger.contains(e.target)`), clicking the toggle button to close the menu can immediately reopen it, creating a frustrating UX.
**Action:** Implement a document-level `click` event listener to close the menu when a user clicks outside the menu container, explicitly ensuring the click target is not the toggle button itself to prevent reopening bugs.
