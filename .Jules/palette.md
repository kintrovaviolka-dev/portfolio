...
## 2026-07-28 - [Dynamic Tooltips for Icon-Only Buttons]
**Learning:** Icon-only buttons (like a clipboard copy button) that change their visual icon upon success (e.g., to a checkmark) often lack immediate feedback for sighted users hovering over them, as the meaning might still be ambiguous without a label.
**Action:** Always provide a native `title` attribute for icon-only buttons to act as a tooltip for sighted users. When the button state changes (e.g., successful copy), dynamically update the `title` attribute to reflect the success state (e.g., 'Copied!') and revert it back along with the visual icon reset.
