...
## 2026-09-04 - [Programmatic Focus Transfer for SPA Hash Navigation]
**Learning:** When navigating via hash links in a single-page application (e.g., `<a href="#about">`), browsers typically scroll to the target section but do not automatically move keyboard focus to it. This leaves keyboard and screen reader users at the top of the page (or wherever focus was before), forcing them to tab through all navigation links again.
**Action:** Intercept hash link clicks, extract the target ID, apply `tabindex="-1"` to the target section, and call `.focus({ preventScroll: true })` programmatically. Ensure the programmatic focus outline on the section wrapper is suppressed in CSS (`section:focus, section:focus-visible { outline: none; }`).
